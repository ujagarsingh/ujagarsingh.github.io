...
1. **target**: हम es5 के बाद से compile करना चाहते हैं जब तक हम browser compatibility के साथ एक package बनाना चाहते हैं।
2. **module**: compatibility के लिए commonjs का उपयोग करें।
3. **declaration**: जब आप package बनाते हैं, तो यह true होना चाहिए। ऐसा करने से Typescript तब compile करते समय  `Javascript code` और  type definitions एक साथ  export करेगा ताकि package का उपयोग Typescript और Javascript दोनों के साथ किया जा सके।
4. **outDir**: Javascript को lib folder में compile किया जाएगा।
5. **include**: सभी source फ़ाइलें `src` folder में  होंगी ।
6. **exclude**: हम node_modules को  transpile नहीं करना चाहते हैं, न ही tests करना चाहते हैं, क्योंकि ये केवल development के दौरान उपयोग किए जाते हैं।
...
चूंकि हम अपने Typescript source-files के को चेक करने के लिए  tests लिख रहे हैं, इसलिए हमें `ts-jest` और `@type/jest` भी जोड़ना होगा।  test suite का उपयोग केवल development के दौरान किया जाता है, इसलिए उन्हें **devDependencies** के रूप में add करना होगा ।

```javascript
npm install --save-dev jest ts-jest @types/jest
```

रुको ! अब हमें **Jest** को configer करने की आवश्यकता है। आप एक `jest` सेक्शन को **package.json** में लिखना या एक अलग config file बनाना चुन सकते हैं। हम इसे एक saperate file में जोड़ने जा रहे हैं, इसलिए जब हम `package` publish करेंगे तो यह शामिल नहीं होगा।

root में एक नई फ़ाइल बनाएँ और इसे **jestconfig.json** नाम दें:
...
Package.json में पुरानी test script निकालें और इसे इसमें बदलें:
...
### Write a basic test
यह हमारी पहली test लिखने का समय है! 

`Src` folder में, `__tests__` नामक एक नया folder बनायें और उसके अंदर, आपको पसंद किए गए नाम के साथ एक new file जोड़ें, लेकिन इसे test.ts के साथ समाप्त होना है, उदाहरण के लिए `Greeter.test.ts`.
...
ठीक है, तो केवल एक चीज जो हम यहां कर रहे हैं वह यह सत्यापित करना है कि यदि इनपुट `Carl` है, तो हमारा method `Greeter` `Hello Carl` return करेगा ।
...
देखो, यह काम करता है! जैसा कि आप देख सकते हैं कि हमने एक test पास की है।

### Use the magic scripts in NPM
एक अच्छे package के लिए, हमें निश्चित रूप से जितना संभव हो उतना automate करना चाहिए। हम npm में कुछ scripts जैसे-: **prepare, prepublishOnly, preversion, version** और **postversion** के बारे में और अधिक जानकारी लेते हैं |

**prepare** तैयार दोनों पहले से ही चलेगा package पैक और प्रकाशित किया गया है, और स्थानीय npm पर स्थापित करें। कोड के निर्माण के लिए बिल्कुल सही। इस स्क्रिप्ट को package.json में जोड़ें

```javascript
"prepare" : "npm run build"
```

**prepublishOnly** पहले से तैयार हो जाएगा और पहले से ही npm प्रकाशित पर तैयार होगा। यहां हम यह सुनिश्चित करने के लिए कि हम खराब कोड प्रकाशित नहीं करेंगे, यह सुनिश्चित करने के लिए हम अपना परीक्षण और एक बार चलेंगे:

```javascript
"prepublishOnly" : "npm test && npm run lint"
```

**preversion** नए package संस्करण को टक्कर देने से पहले प्रबलता चलेगी। यह सुनिश्चित करने के लिए कि हम खराब कोड वाले संस्करण से नहीं टकरा रहे हैं, यहाँ भी क्यों नहीं चला?
...
**Postversion** कमिटमेंट के बाद पोस्टवॉर्शन चलेगा। टैग के साथ-साथ कमिट को आगे बढ़ाने के लिए एक सही जगह।

```javascript
"postversion" : "git push && git push --tags"
```

**package.json** में मेरी script section इस प्रकार है:
...
हमारे अच्छे package को खत्म करने का आखिरकार समय आ गया है! सबसे पहले, हमें अपने **package.json** में कुछ बदलाव करने की जरूरत है। फिर से देखें:
...
यदि आपके पास कोई खाता नहीं है तो आप [https://www.npmjs.com/signup](https://www.npmjs.com/signup){:target='_blank'} पर ऐसा कर सकते हैं
या कमांड चलाएँ: `npm adduser`
